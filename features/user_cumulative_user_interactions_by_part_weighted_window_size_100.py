...
class UserCumulativeUserInteractionsByPartWeightedWindowSize100(BaseFeature):
        ...
        query = """
            ...
            cumulative AS (
              SELECT
                ...
                SUM(aggregation_per_content.weight) OVER (PARTITION BY user_id, part ORDER BY timestamp ROWS BETWEEN 100 PRECEDING AND 1 PRECEDING) AS cumlative_user_appearance,
                SUM(answered_correctly * aggregation_per_content.weight) OVER (PARTITION BY user_id, part ORDER BY timestamp ROWS BETWEEN 100 PRECEDING AND 1 PRECEDING) AS cumlative_user_corrected_answers,
              FROM
...
if __name__ == "__main__":
    UserCumulativeUserInteractionsByPartWeightedWindowSize100.main()
